...
//    private static Parser parser = Parser.builder().build();
     ...
     * @param uid
     */
    public static void setCookie(HttpServletResponse RestResponse, Integer uid) {
        try {
        ...
        if (null != request) {
            ...
            if (cookie != null && cookie.getValue() != null) {
                try {
                    ...
                    return !StringUtils.isEmpty(uid) && Tools.isNumber(uid) ? Integer.valueOf(uid) : null;
                } catch (Exception e) {
                    logger.error("aes error:{}", e.getMessage());
                }
            }
        }
        return null;
...
//            默认是classPath路径
    ...
    /*public static void logout(HttpSession session, HttpServletResponse RestResponse) {
        session.removeAttribute(WebConst.LOGIN_SESSION_KEY);
        Cookie cookie = new Cookie(WebConst.USER_IN_COOKIE, "");
        cookie.setMaxAge(0);
        RestResponse.addCookie(cookie);
        try {
            RestResponse.sendRedirect(Commons.site_url());
        } catch (IOException e) {
